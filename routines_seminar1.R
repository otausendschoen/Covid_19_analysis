...
lassopost  <- function(y,x, method.lambda='bic', verbose = FALSE) {
  ...
  lcv= fixedLassoInf(x=xstd,y=ystd,beta=b,lambda=lambda*nrow(x),sigma=sigmahat,verbose=verbose,tol.beta=1e-10)
  ...
  ci.lassopost[abs(b) >= 1e-10,]= cbind(lcv$coef0, lcv$ci, lcv$pv)
  ci.lassopost[abs(b) < 1e-10,4]= NA
  return(ci.lassopost)
}
